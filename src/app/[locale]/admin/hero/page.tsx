...
export default function HeroAdminPage() {
  ...
  return (
    <div className="bg-[#111111] p-6 rounded-lg shadow-sm border border-gray-800">
      <h1 className="text-2xl font-bold text-white mb-6">Hero Section Settings</h1>
      
      {message && (
        <div className={`p-4 mb-6 rounded border ${message.includes('successfully') ? 'bg-green-900/40 text-green-300 border-green-800' : 'bg-red-900/40 text-red-300 border-red-800'}`}>
          {message}
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="space-y-6 max-w-2xl">
        <div className="grid grid-cols-1 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-300">Hero Main Title</label>
            <input
              dir="ltr"
              type="text"
              value={heroTitleEn}
        ...
        <div className="grid grid-cols-1 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-300">Hero Subtitle / Description</label>
            <textarea
              dir="ltr"
